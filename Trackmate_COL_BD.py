...
def main(): 
    ...
    csv_path = 'F:\\DiffusionAnalysis\\test' # 
    rs_path = 'F:\\DiffusionAnalysis\\test' # *.tif.RESULT
    mask_path = 'F:\\DiffusionAnalysis\\test' # *.png
    max_frame_gap = 4
    max_distance_gap = 2
                ...
                for entryB in trB:
                    ...
                    for entryL in trL:
                        ...
                        if(rsS[k] == None):
                            ...
                            rs_index = fit_track_to_rs(trackBound, rsS[k])
                        for entryF in fit:
                            if(len(entryF) > 4):
                                entryF[4] = rs_index
                        trL[entryL] = track_insert(trackLife, fit)
                        n_fit += 1
                        break
...
def fit_bound_to_track(bound, life, frame_gap, distance_gap):
    ...
    while True:
        ...
        if(life[l][0] != frame and bound[b][0] != frame):
            ...
            if(d1 > distance_gap or d2 > distance_gap):
                break
            else: gap = 0
            record = (life[l][1], life[l][2])
            entry = life[l].copy()
...
def csv_name_sort_loose(path:str):
    ...
    for file in flist:
        ...
        if 'spotsBound' in fname[-1]:
            sB.append(file)
        elif 'spotsLifetime' in fname[-1]:
            sL.append(file)
        elif 'tracksBound' in fname[-1]:
            tB.append(file)
        elif 'tracksLifetime' in fname[-1]:
            tL.append(file)
