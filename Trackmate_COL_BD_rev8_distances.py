...
def main():
    # Note: rs -> fixed particle location
    ...
    log_file_res = csv_path + '\\_ColBD_RESULT.txt'
    ...
    for i in range(len(masks)):
        ...
        for k in range(n_cell):
            ...
            if (spotsBound == None):  # Label as Diffusive for all
                ...
                for entryB in trB:
                    ...
                    for c in range(len(trL_aligned)):
                        ...
                        if ((frB_max > trackLife[0][0] or frB_min < trackLife[-1][0]) and len(
                                trackLife) > min_lifetrack):
                            fit = fit_bound_to_track(trackBound, trackLife, max_frame_gap, max_distance_gap,
                                                     entry_tolerance)
            ...
            for entryL in trL:
                ...
                if(len(rebinds) > 0):
                    rebinds = rebind_particle_distance(rebinds, rsS[k])
                if(len(rb_events) > 0):
    ...
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[6] for entry in final_rebind]))
    print_log('\t\t\tstd:', np.std([entry[6] for entry in final_rebind]))
    print_log('')
    print_log('\tRebinding to Everything | without single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_purge_single))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[6] for entry in final_rebind_purge_single]))
    print_log('\t\t\tstd:', np.std([entry[6] for entry in final_rebind_purge_single]))
    print_log('')
    print_log('\tRebinding to Fixed Particles | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_only_rs))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[6] for entry in final_rebind_only_rs]))
    print_log('\t\t\tstd:', np.std([entry[6] for entry in final_rebind_only_rs]))
    print_log('')
    print_log('\tRebinding to Fixed Particles (SAME) | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_same))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[6] for entry in final_rebind_same]))
    print_log('\t\t\tstd:', np.std([entry[6] for entry in final_rebind_same]))
    print_log('')
    print_log('\tRebinding to Fixed Particles (DIFFERENT) | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_diff))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[6] for entry in final_rebind_diff]))
    print_log('\t\t\tstd:', np.std([entry[6] for entry in final_rebind_diff]))
    print_log('')
    print_log('------------------------------------------------------------------------------------------')
    print_log('')
    ...
    final_rprob_purge_single = float(len(final_rebind_purge_single)) / float(
        len(final_rebind_purge_single) + final_rebind_unsuccessful_count)
    ...
    with open(log_file) as fin, open(log_file_res, 'w') as fout:
        active = False
        for line in fin:
            if '# Track Bound:' in line:
                active = True
            if active:
                fout.write(line)
...
# format:
#   [number, rs_prev, rs_next, rebinding time, avg speed (pix/fr), distance_between_bound, x1, y1, x2, y2]
...
# Determine the distance to each particle in the cell,
# replace positions to particle distance [begin -> particle, end -> particle] for each particle
def rebind_particle_distance(rebinds, particles):
    # no particle
    if(len(particles) == 0):
        for i in range(len(rebinds)):
            rebinds[i] = rebinds[i][:-4]
            rebinds[i][1], rebinds[i][2] = 0, 0
        return rebinds

    # one or more particle, iterate
    for i in range(len(rebinds)):
        event = rebinds[i].copy()
        start, end = event[-4:-2], event[-2:]
        rebinds[i] = rebinds[i][:-4]
        for particle in particles:
            rebinds[i] += [distance(start, particle[2:4]), distance(end, particle[2:4])]
    return rebinds
...
def track_insert(track, insert):
    ...
    inserted = False
    for entry in track:
        if (entry[0] < min(frames) or entry[0] >= max(frames)):
            res.append(entry)
        else:
            if (not inserted):
                res += insert
