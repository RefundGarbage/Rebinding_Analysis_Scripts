...
def main(): 
    ...
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[-7] for entry in final_rebind]))
    print_log('\t\t\tstd:', np.std([entry[-7] for entry in final_rebind]))
    print_log('')
    print_log('\tRebinding to Everything | without single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_purge_single))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[-7] for entry in final_rebind_purge_single]))
    print_log('\t\t\tstd:', np.std([entry[-7] for entry in final_rebind_purge_single]))
    print_log('')
    print_log('\tRebinding to Fixed Particles | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_only_rs))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[-7] for entry in final_rebind_only_rs]))
    print_log('\t\t\tstd:', np.std([entry[-7] for entry in final_rebind_only_rs]))
    print_log('')
    print_log('\tRebinding to Fixed Particles (SAME) | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_same))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[-7] for entry in final_rebind_same]))
    print_log('\t\t\tstd:', np.std([entry[-7] for entry in final_rebind_same]))
    print_log('')
    print_log('\tRebinding to Fixed Particles (DIFFERENT) | with single-frame events:')
    print_log('\t\t-> # Rebinding Event =', len(final_rebind_diff))
    print_log('\t\t-> Average Rebind Time:', np.mean([entry[-7] for entry in final_rebind_diff]))
    print_log('\t\t\tstd:', np.std([entry[-7] for entry in final_rebind_diff]))
    print_log('')
    print_log('------------------------------------------------------------------------------------------')
    print_log('')
